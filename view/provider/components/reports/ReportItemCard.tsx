        ...
        >
            <div className={`flex-1 ${layoutMode === 'list' ? 'min-w-0' : ''}`}>
                <div className="flex justify-between items-start mb-2">
                    ...
                    {layoutMode === 'grid' && (
                        <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase shrink-0 ${report.status === 'handled' ? 'bg-green-100 text-green-600' : 'bg-orange-100 text-orange-600'}`}>{report.status}</span>
                    )}
                </div>
            ...
            {layoutMode === 'list' && (
                <div className="flex flex-col items-end gap-2 shrink-0">
                    <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${report.status === 'handled' ? 'bg-green-100 text-green-600' : 'bg-orange-100 text-orange-600'}`}>{report.status}</span>
