...
export const QualityCheckInventoryInput: React.FC<QualityCheckInventoryInputProps> = ({ onAnalysisComplete, onBack }) => {
  ...
  const runAnalysis = async (img: string) => {
    try {
        ...
        // Menghitung total berat untuk kalkulasi dampak sosial AI
  ...
  if (step === 'loading') {
      return (
        <div className="flex flex-col items-center justify-center py-24 text-center animate-in fade-in">
            ...
            <h3 className="text-3xl font-black text-stone-900 dark:text-white italic">AI Auditor Sedang Mengecek...</h3>
            <p className="text-stone-500 mt-3 max-w-sm mx-auto">Kami menganalisis kesegaran dan kelayakan makanan donasi Anda.</p>
        </div>
      );
  }
      ...
      return (
        <div className="w-full max-w-lg mx-auto py-12 animate-in zoom-in-95">
             <div className="relative group cursor-pointer w-full max-w-xs aspect-square mx-auto border-4 border-dashed border-orange-200 dark:border-stone-700 rounded-[2.5rem] flex flex-col items-center justify-center hover:border-orange-500 hover:bg-orange-50/30 transition-all duration-500">
                ...
                <span className="font-bold text-xl text-stone-800 dark:text-stone-200">Ambil/Upload Foto</span>
                <span className="text-sm text-stone-400 mt-2 px-6 text-center">AI membutuhkan foto asli makanan untuk verifikasi kualitas</span>
            </div>
  ...
  return (
    <div className="w-full max-w-2xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {/* Input Berat per Porsi/Box jika satuannya porsi/box */}
        ...
        {/* Pengaturan Batas Pengambilan per Penerima */}
        ...
        <div className="grid grid-cols-2 gap-4">
            <Input label="Waktu Masak/Beli" type="datetime-local" value={form.madeDateTime} onChange={e => setForm({...form, madeDateTime: e.target.value})} className="!bg-stone-100 dark:!bg-stone-800" />
            <div className="space-y-2">
        ...
        <div className="space-y-2 p-6 bg-orange-50 dark:bg-orange-900/10 rounded-2xl border border-orange-100 dark:border-stone-800">
            ...
            <p className="text-[10px] text-orange-600 dark:text-orange-400 italic mt-1">*Default disetel pada waktu sibuk surplus: 18:30 - 21:00</p>
        </div>
